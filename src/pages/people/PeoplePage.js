...
import { setPeople } from "../../store/features/PeopleSlice";
...
const PeoplePage = () => {
  ...
  const handleView = (data) => {
    navigate(`/people/${data?.user_id}`, { state: data });
  };

  return (
    <>
    <HelmetComponent title={`${EDISPATCHED} | People `} />
      <Container maxWidth="xl">
        ...
        >
          ...
          <CustomButton
            ...
            onClick={() => navigate("/people/add-people")}
          />
        </Stack>
                  ...
                  {Array.isArray(people) &&
                    people?.map((user, index) => (
                      <TableRow hover key={index}>
                        ...
                        <TableCell align="center">{user?.user_tenant_profile[0]?.role}</TableCell>
                        
                        <TableCell align="center">
                          <Label
                            color={renderChipColorByStatus(user?.user_tenant_profile[0]?.status)}
                          >
